...
class display:
    def __init__(self, rows, cols, times, question_size, level_up_thresh, start, diff, diff_inc, diff_change):
        self.rows = rows
        self.cols = cols
    ...
    def countdown(self):
        ...
        while not self.is_answer_entered and self.time_left > 0.0:  
            # self.show_questions()
            # print("Answer|Time")      
            print(f"      |{int(self.time_left // 1)}", end = '\r')     
            for i in range(100):              
    ...
    def show_questions(self):   
        ...
        print(f"Correct Answers: {self.num_correct_ans} | Attempts: {self.attempts}")
    ...
    def answer_question(self):
        ...
        if before_ans == self.num_correct_ans:
